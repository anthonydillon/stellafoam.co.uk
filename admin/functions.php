...
function addStock($name, $code, $type, $pack, $discount, $price, $colour, $inStock){
	echo($type +' - ' + $pack+ ' - '+ $discount);
	$con = mysql_connect(db_hostname,db_username,db_password);
	if (!$con) {
	  die('Could not connect: ' . mysql_error());
	}
	mysql_select_db(db_name, $con);
	$SQL = "INSERT INTO stock (Stock_Name, Stock_Code, Type_Name, Pack, Discount, Price, Product_ID, In_Stock) VALUES ('".$name."','".$code."','".$type."','".$pack."','".$discount."','".$price."',".$colour.",".$inStock.")";
...
function editStock($id, $name, $code, $type, $pack, $discount, $price, $colour,$inStock){
	$con = mysql_connect(db_hostname,db_username,db_password);
	if (!$con) {
	  die('Could not connect: ' . mysql_error());
	}
	...
	$SQL = "UPDATE stock SET Stock_Name = '".$name."', Stock_Code = '".$code."', Type_Name = '".$type."', Pack = '".$pack."', Discount = '".$discount."', Price = '".$price."', Product_ID = '".$colour."', In_Stock = '".$inStock."' WHERE Stock_ID = '".$id."'";
	$result = mysql_query($SQL);
